...
asyncTest("Feature Info is shown", 3, function() {
    ...
    mq.layers()[0].bind('loadend', function(evt, data) {
        ...
        start();
        equals($('#info1').find('p').length, 1, 'Popup contains data');
        equals($('#info1').find('p').text(), 'poly1-1', 'Feature 1 was selected');

        feature = data.object.features[1];
        layer.events.triggerEvent('featureselected', {feature: feature});
        equals($('#info1').find('p').text(), 'poly1-2', 'Feature 2 was selected');
...
asyncTest("Feature Info bound to two nodes", 5, function() {
    ...
    mq.layers()[0].bind('loadend', function(evt, data) {
        ...
        layer.events.triggerEvent('featureselected', {feature: feature});
        start();
        equals($('#info2').find('p').length, 1, 'Popup contains data');
